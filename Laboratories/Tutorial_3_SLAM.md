...
La sección 5 sirve para revisar el mapa una vez, a mano. Si en cambio querés tenerlo **de fondo mientras manejás o probás cosas** (sin mapear de nuevo), este repo ya incluye un launch file (`src/controllers/launch/bridge_with_map.launch.py`) que levanta bridge + RViz + el mapa ya activado, todo en un solo comando — en vez de escribir 5 comandos en 3 terminales cada vez. Ya se copió a tu workspace junto con el resto de `src/controllers/` en el paso "Cómo usar este repo" del `README.md`, así que no hay nada que crear.
...
Por qué tiene un delay de 5 segundos antes de arrancar `map_server`: publica `/map` una sola vez, al activarse — si RViz todavía no se terminó de suscribir en ese instante, se pierde ese mensaje para siempre (el display `Map` de RViz usa QoS `VOLATILE`, que no recibe el histórico cacheado de un publisher `TRANSIENT_LOCAL` como `map_server`, solo lo que se publique después de que ya está suscripto). El delay le da tiempo a RViz de arrancar y suscribirse antes de que salga esa única publicación — sin él, es una carrera y a veces el mapa nunca aparece ("No map received" en el display, sin ningún error de código de por medio).
...
### 6.2 Agregar el display una vez y guardarlo para no repetirlo
